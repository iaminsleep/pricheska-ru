...
<section class="modal completion-form form-modal" id="complete-form">
    ...
    <form action="{{ route('task.complete', ['taskId' => $taskId]) }}" method="post">
        ...
        <input class="visually-hidden completion-input completion-input--difficult" type="radio"
            id="completion-radio--yet" name="status" value="3">
        ...
        @if ($performerId)
            <p>
                ...
                <textarea class="input textarea" rows="4" id="completion-comment" name="comment"
                    placeholder="Напишите отзыв о качестве работе исполнителя"></textarea>
            </p>
            ...
            <div class="feedback-card__top--name completion-form-star">
                @for ($i = 1; $i <= 5; $i++)
                    <span class="star-disabled">
                        <input type="radio" name="rating" style="opacity: 0; cursor: pointer"
                            value="{{ $i }}" />
                    </span>
                @endfor
            </div>
